...
export class FakeToiletSocket {
  ...
  // 현실적 체류시간: 대부분 1~3분, 5분 이상은 탭 방치 케이스
  _randomDwellSec() {
    const r = Math.random();
    if (r < 0.40) return 30 + Math.random() * 60;    // 30~90초 (40%)
    if (r < 0.70) return 90 + Math.random() * 90;    // 90~180초 (30%)
    if (r < 0.88) return 180 + Math.random() * 120;  // 180~300초 (18%)
    if (r < 0.96) return 300 + Math.random() * 300;  // 300~600초 (8%)
    return 600 + Math.random() * 600;                 // 600~1200초 (4%, 탭 방치)
  }
  _pick(arr) {
