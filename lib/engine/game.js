...
export function initGame() {
  ...
  const LIGHT_BLACKOUT_DURATION = 8000; // 암전 지속 시간
  const LIGHT_BLACKOUT_REACTION_DELAY = 2500; // 암전 후 옆 칸 놀람 반응
  const LIGHT_RECOVERY_REACTION_DELAY = 900; // 복구 후 옆 칸 안도 반응
  ...
  // ===== 이스터에그: 문고리 =====
  const LATCH_EASTER_TAPS = 6; // 10초 안에 6번 → 박살
  const LATCH_EASTER_WINDOW_MS = 10000;
  const LATCH_BREAK_REACTION_DELAY = 1500; // 박살 후 옆 칸 반응
  ...
  let suppressBotChatUntil = 0; // 이스터에그 연출 중 일반 봇 채팅 억제
  let latchEasterTaps = [];
  ...
  socket.on("chat", (msg) => {
    ...
    if (msg.kind === "bot" && Date.now() < suppressBotChatUntil) return; // 이스터에그 연출 중 억제
    showBubble({
      name: msg.name,
      text: msg.text,
  ...
  socket.on("flush", (f) => {
    ...
    if (f.kind === "bot" && Date.now() < suppressBotChatUntil) return; // 이스터에그 연출 중 억제
    showBubble({
      name: f.name,
  ...
  function tearToiletPaper() {
    ...
    if (tpPullCount >= TP_MAX_PULLS) {
      ...
      tpEmpty = true;
      suppressBotChatUntil = Math.max(suppressBotChatUntil, Date.now() + TP_REFILL_MS);
  ...
  function triggerBlackout() {
    ...
    suppressBotChatUntil = Math.max(suppressBotChatUntil, Date.now() + LIGHT_BLACKOUT_DURATION + 3000);
  ...
  function tapCeilingLight() {
    ...
    if (
      ...
      lightEasterTaps = [];
      triggerBlackout(); // 6번째 탭 = 암전 트리거 — blink 없이 바로 꺼짐
    } else {
      replayAnim(skinLight, "skin-light--blink"); // 1~5번째 탭만 깜빡임
    }
  }
