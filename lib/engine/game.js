...
export function initGame() {
  ...
  function restoreAwayProgress() {
    ...
    if (newSat >= MAX_SESSION_SECONDS) {
      ...
      toast('⏰ 자리 비운 사이 <span class="toast-em toast-em--max">MAX</span> 달성!', true);
    } else if (awaySec >= 300) {
      // 5분 이상 자리비움 시에만 토스트
      const awayMin = Math.max(1, Math.round(awaySec / 60));
      revealTimer(); // 부재중 적립 — 시계 ON
      toast(`💰 부재중 자동 적립! <span class="toast-em toast-em--gain">+${awayMin}분</span>`, true);
    }
  }
  ...
  /* ---------- 이스터에그: 천장 형광등 30탭 → 개발자 도구 노출 ----------
     일반 사용자에겐 설정의 개발자 도구가 숨겨져 있고, 형광등을 사람이 칠 수 있는 속도로
     30번 연타하면(텀이 너무 벌어지면 리셋) 노출된다. 해제 상태는 저장(다음부터 바로 보임). */
  const skinLight = $("skinLight");
  const LIGHT_UNLOCK_TAPS = 30;
  const LIGHT_TAP_RESET_MS = 2000; // 직전 탭과 이 간격 이상이면 카운트 리셋(연속 탭만 인정)
  let lightTaps = 0;
  let lightTapAt = 0;
  function tapCeilingLight() {
    if (LS.getBool(KEY.devToolsUnlocked)) return; // 이미 해제됨
    const now = Date.now();
    lightTaps = now - lightTapAt <= LIGHT_TAP_RESET_MS ? lightTaps + 1 : 1;
    lightTapAt = now;
    // 막바지(거의 다 옴)엔 살짝 깜빡여 '뭔가 있다'는 피드백
    if (lightTaps >= LIGHT_UNLOCK_TAPS - 4 && lightTaps < LIGHT_UNLOCK_TAPS) {
      replayAnim(skinLight, "skin-light--blink");
    }
    if (lightTaps >= LIGHT_UNLOCK_TAPS) {
      LS.setBool(KEY.devToolsUnlocked, true);
      if (el.devTools) el.devTools.hidden = false;
      skinLight.classList.add("skin-light--on");
      toast("🛠 개발자 도구가 켜졌어요");
    }
  }
  if (skinLight) skinLight.addEventListener("click", tapCeilingLight);
  // 이미 해제된 기기면 처음부터 노출 + 불 켜진 상태 유지
  if (LS.getBool(KEY.devToolsUnlocked)) {
    if (el.devTools) el.devTools.hidden = false;
    if (skinLight) skinLight.classList.add("skin-light--on");
  }
