...
export function initGame() {
  ...
  const LIGHT_EASTER_DEBOUNCE_MS = 3000;
  ...
  const LATCH_EASTER_DEBOUNCE_MS = 3000;
  ...
  // ===== 이스터에그: 휴지 =====
  const TP_MAX_PULLS = 150; // 150번 소진 시 빈 상태
  const TP_REFILL_MS = 20000; // 20초 후 리필
  const TP_FULL_R = 27; // 꽉 찬 롤 반지름
  const TP_CORE_R = 7; // 심(cardboard) 반지름
  ...
  // 개발자 후원/공유하기/광고문의 토스트 — 토스트 표시 중이면 순차, 사라졌으면 처음부터
  let donateToastIdx = 0;
  let donateToastActive = false;
  let donateToastActiveTimer = null;
  let adToastIdx = 0;
  ...
  function showDonateToast() {
    if (!donateToastActive) donateToastIdx = 0;
    toast(DONATE_TOASTS[donateToastIdx % DONATE_TOASTS.length]);
    donateToastIdx++;
    donateToastActive = true;
    clearTimeout(donateToastActiveTimer);
    donateToastActiveTimer = setTimeout(() => { donateToastActive = false; }, TOAST_DURATION + 100);
  }
  ...
  function openAdInquiry() {
    ...
    adToastActiveTimer = setTimeout(() => { adToastActive = false; }, TOAST_DURATION + 100);
  }
  ...
  el.adA4.addEventListener("click", (e) => {
    ...
    else showDonateToast(); // 광고 없으면 개발자 후원하기와 동일
  });
  ...
  return () => {
    ...
    if (donateToastActiveTimer) clearTimeout(donateToastActiveTimer);
