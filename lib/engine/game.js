...
export function initGame() {
  ...
  // ===== 이스터에그: 휴지 =====
  const TP_MAX_PULLS = 100; // 100번 소진 시 빈 상태
  ...
  function handleLatchClick() {
    ...
    if (latchEasterBotTimer) {
      ...
      latchEasterBotTimer = setTimeout(fireLatchEasterBot, LATCH_EASTER_DEBOUNCE_MS);
      return;
    }
  ...
  function tapCeilingLight() {
    ...
    if (lightEasterBotTimer) {
      ...
      lightEasterBotTimer = setTimeout(fireLightEasterBot, LIGHT_EASTER_DEBOUNCE_MS);
      return;
    }
