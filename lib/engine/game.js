...
export function initGame() {
  ...
  let flushReactionPools = {
    shortSmall: [...FLUSH_REACTIONS.shortSmall],
    shortLarge: [...FLUSH_REACTIONS.shortLarge],
    longSmall: [...FLUSH_REACTIONS.longSmall],
    longLarge: [...FLUSH_REACTIONS.longLarge],
  };
  let flushReactionNickPool = [...FLUSH_REACTION_NICKS];
  let flushReactionTimer = null;
  let flushReactionPending = false;
  // 개발자 도구 탭 카운터
  let devUnlockTaps = 0;
  ...
  function maybeRevealReceipt() {
    ...
    setTimeout(() => { receiptRevealQuiet = true; }, Math.max(0, RECEIPT_SHOW_DELAY - 3000));
    setTimeout(() => {
      ...
      receiptRevealQuietTimer = setTimeout(() => { receiptRevealQuiet = false; }, 3000);
    }, RECEIPT_SHOW_DELAY);
  }
  ...
  function commitFlush(amount, chatText, capped) {
    ...
    recordFlush(amount);
    rotateReceiptSlogan();
    const satSecsSnapshot = state.satSeconds; // resetTimer 전에 캡처
    maybeShowFlushReaction(amount, satSecsSnapshot);
  ...
  socket.on("chat", (msg) => {
    ...
    if (msg.kind === "bot" && (flushReactionPending || receiptRevealQuiet)) return; // 리액션 대기 중 / 영수증 넛지 조용히
    showBubble({
      name: msg.name,
      text: msg.text,
  ...
  socket.on("flush", (f) => {
    ...
    if (f.kind === "bot" && (flushReactionPending || receiptRevealQuiet)) return; // 리액션 대기 중 / 영수증 넛지 조용히
    showBubble({
      name: f.name,
  ...
  function updateChatPlaceholder() {
    ...
    el.chatNick.classList.toggle(
      "chat__nick--reroll",
      !state.nickPinned && state.nickIsAuto,
    );
  }
  ...
  // 내 물내림 후 옆칸 봇 리액션 — 1인 25%, 1명 추가될 때마다 2%p씩 감소 (최소 5%)
  // 대기 중엔 flushReactionPending=true로 배경 봇 채팅을 억제해 혼잡도를 낮춘다.
  function maybeShowFlushReaction(amount, satSec) {
    const prob = Math.max(0.05, 0.25 - (state.stalls - 1) * 0.02);
    ...
    const poolKey =
      satSec < 90
        ...
        : amount < 3000
          ? "longSmall"
          : "longLarge";
    clearTimeout(flushReactionTimer);
    flushReactionPending = true;
    flushReactionTimer = setTimeout(
      () => {
        flushReactionPending = false;
        const text = pickFromPool(
          flushReactionPools[poolKey],
          FLUSH_REACTIONS[poolKey],
        );
        const name = pickFromPool(flushReactionNickPool, FLUSH_REACTION_NICKS);
        showBubble({ name, text, isBot: true });
      },
  ...
  return () => {
    ...
    if (receiptRevealQuietTimer) { clearTimeout(receiptRevealQuietTimer); receiptRevealQuiet = false; }
