...
export function initGame() {
  ...
  // ===== 이스터에그: 문고리 =====
  const LATCH_EASTER_TAPS = 5;              // 열림 시작 → 홀수 5번 → 잠김 상태에서 박살
  const LATCH_EASTER_WINDOW_MS = 10000;
  const LATCH_BREAK_DELAY = 500;            // 잠김 확정 후 박살까지 딜레이
  const LATCH_BREAK_REACTION_DELAY = 2000;  // 박살 후 옆 칸 반응
  ...
  function renderHistoryBtns() {
    ...
    el.receiptBtnSettings.classList.toggle("settings__btn--no-record", noRecord);
  ...
  function togglePanel(name) {
    ...
    if (panel.hidden) {
      ...
      const isSalaryFirst = name === "salary" && !LS.getBool(KEY.salaryFocusDone);
      if (isSalaryFirst) {
        ...
        // 이후 오픈: 패널 자체 미세 등장
        replayAnim(panel, name === "salary" ? "salary-panel--in" : "settings-panel--in");
      }
  ...
  function triggerLatchBreak() {
    // 잠김 상태 보장(5번째 탭에서 open 클래스가 제거됨)
  ...
  function recoverFromBlackout() {
    ...
    if (blackoutOverlayEl) {
      blackoutOverlayEl.classList.add("blackout-overlay--out");
      const overlay = blackoutOverlayEl;
      blackoutOverlayEl = null;
  ...
  function triggerBlackout() {
    ...
    blackoutOverlayEl.className = "blackout-overlay";
    sceneEl.appendChild(blackoutOverlayEl);
    ...
    lightBlackoutTimer = setTimeout(recoverFromBlackout, LIGHT_BLACKOUT_DURATION);
  }
  ...
  /* ---------- 시작 ---------- */
  loadAll();
  // 문고리 초기 상태: 열림(화장실 방금 들어온 상태) — 홀수 탭으로 잠기고 5번째에 박살
  if (skinLatch) skinLatch.classList.add("skin-latch--open");
  initSalaryHint();
  ...
  rotateAd();
  let adInterval = null;
  const adStartTimer = AD_CREATIVES.length > 1 ? setTimeout(() => {
    adInterval = setInterval(rotateAd, AD_ROTATE_MS);
  }, AD_ROTATE_START_DELAY) : null;
  ...
  return () => {
    ...
    if (blackoutOverlayEl) { blackoutOverlayEl.remove(); blackoutOverlayEl = null; }
