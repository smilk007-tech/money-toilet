...
export function initGame() {
  ...
  function commitFlush(amount, chatText, capped) {
    ...
    snapshotTier(); // 이 물내림 시점의 루팡 티어 고정 — 새 명세서 도장·델타의 근거
    rotateReceiptSlogan();
  ...
  function openResetConfirm() {
    ...
    // "오늘 루팡시간" — 초기화로 지워지는 값(티어 근거)이라 세션 타이머 대신 이걸 보여준다
    el.resetConfirmTime.textContent = formatDwellTime(
      Math.round(state.todayDwellSec),
    );
  ...
  function openReceipt() {
    ...
    closePanels();
    // prevP(직전 티어)·todaySec(오늘 루팡시간)은 인게임 모달 전용 메타 —
    // PayslipModal이 분리 수거하며, 공유 URL 인코딩에는 절대 포함되지 않는다(체류시간 역산 방지).
    window.dispatchEvent(
      new CustomEvent(APP_EVENTS.payslipOpen, {
        detail: {
          ...buildReceiptData(),
          prevP: currentTierInfo().prevP,
          todaySec: state.todayDwellSec,
        },
      }),
    );
  }
