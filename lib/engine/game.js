...
export function initGame() {
  ...
  // ===== 이스터에그: 휴지 =====
  const TP_MAX_PULLS = 8;    // 8번 소진 시 빈 상태
  ...
  // 롤 원 요소 참조 (SVG 첫 번째 circle = 흰색 휴지 롤)
  const tpRollCircle = skinTp?.querySelector("circle");
  ...
  function tearToiletPaper() {
    ...
    // 일반 뽑기 — 클릭마다 즉시 얇아짐, 자동 재충전 없음(직접 클릭해야 다음 장 뽑힘)
    dropTpSheet();
  }
  ...
  if (skinLight) skinLight.addEventListener("click", tapCeilingLight);
  ...
  return () => {
    ...
    if (skinLight) skinLight.removeEventListener("click", tapCeilingLight);
