...
export function initGame() {
  ...
  function showBubble(msg, forceSide) {
    ...
    // 스타일(kind)과 "봇 표시(🤖 접두사)"는 서로 독립 — 봇 채팅/물내림도 일반 말풍선과 똑같이 보이고,
    // 닉네임 맨 앞에 🤖만 붙는다(이전의 회색/이탤릭 처리는 가독성 문제로 제거).
    const kindCls =
      msg.kind === "capped"
        ? " bubble--capped"
        : msg.kind === "flush"
    ...
    // 봇은 닉네임 앞에 붙이면 가독성이 떨어져서, 닉네임 줄 맨 오른쪽 끝에 🤖만 배치(닉네임은 그대로 잘 보이게)
    const label =
      msg.kind === "capped"
        ? "💩 " + safe(msg.name)
        : msg.kind === "admin"
    ...
    const nameRow = msg.isBot
      ? `<b style="display:flex;justify-content:flex-start;align-items:center;gap:3px">${label}<span style="font-size:0.8em; opacity:0.5">🤖</span></b>`
      : `<b>${label}</b>`;
