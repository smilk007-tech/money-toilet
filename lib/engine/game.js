...
export function initGame() {
  ...
  const RECEIPT_REVEAL_DELAY = 20_000; // 세션 진입 후 20초 지나야 영수증 노출대상(공유 깔때기 단축)
  ...
  // ===== 이스터에그: 천장 형광등 =====
  const LIGHT_EASTER_TAPS = 10;
  const LIGHT_EASTER_WINDOW_MS = 10000;
  const LIGHT_EASTER_COOLDOWN_MS = 30000;
  const LIGHT_EASTER_BOT_DELAY_MS = 2000;
  ...
  // ===== 이스터에그: 문고리 =====
  const LATCH_EASTER_TAPS = 10;
  const LATCH_EASTER_WINDOW_MS = 10000;
  const LATCH_EASTER_COOLDOWN_MS = 30000;
  const LATCH_EASTER_BOT_DELAY_MS = 2000;
  ...
  // ===== 이스터에그: 휴지 =====
  const TP_MAX_PULLS = 50;   // 50번 소진 시 빈 상태
  const TP_REFILL_MS = 10000; // 10초 후 리필
  const TP_FULL_R = 27;       // 꽉 찬 롤 반지름
  const TP_CORE_R = 9;        // 심(cardboard) 반지름
  const TP_REFILL_MESSAGES = [
    "옆 칸님 휴지 배달왔습니다ㅋㅋ",
    "휴지 좀 아껴쓰세요ㅋㅋ 불쌍해서 드림",
    "휴지 떨어졌다길래 드립니다~",
    "휴지 아래로 전달해드려요ㅋㅋ",
  ];
  // ===== 개발자 도구 진입: 우상단 '오늘 다같이' 영역 20번 빠르게 클릭 =====
  const DEV_UNLOCK_TAPS = 20;
  const DEV_TAP_RESET_MS = 2000;
  // ===== 개발자 후원 / 공유하기 / 광고문의 순차 토스트 =====
  const DONATE_TOASTS = [
    "커피 한 잔이 큰 힘이 됩니다 ☕",
    "개발자가 세 시간 더 코딩합니다 💻",
    "소액도 정말 감사합니다 🙏",
    "moneytoilet.kr - 돈버는 화장실",
  ];
  const SHARE_TOASTS = [
    "공유하기가 곧 후원입니다!",
    "5분 쉬었으면 충분합니다~",
  ...
  // 개발자 후원/공유하기/광고문의 토스트 인덱스
  let donateToastIdx = 0;
  let shareToastIdx = 0;
  let adToastIdx = 0;
  ...
  // 부재중 적립/ MAX 도달 안내 시 시계를 강제로 켜준다(이미 켜져 있으면 그대로).
  // 자리 비운 시간·동결 상태를 시계로 바로 확인하게 하기 위함.
  ...
  function openResetConfirm() {
    ...
    el.resetConfirmTime.textContent = formatDwellTime(Math.round(Math.min(state.satSeconds, MAX_SESSION_SECONDS)));
  ...
  el.donateBtn.addEventListener("click", () => {
    toast(DONATE_TOASTS[donateToastIdx % DONATE_TOASTS.length]);
    donateToastIdx++;
  });
  ...
  if (el.settingsShareBtn) {
    el.settingsShareBtn.addEventListener("click", async () => {
      // 순차 토스트 로테이션
      toast(SHARE_TOASTS[shareToastIdx % SHARE_TOASTS.length]);
      shareToastIdx++;
      // 공유 기능도 병행 (URL 복사 또는 네이티브 공유)
  ...
  const tpRollCircle = skinTp?.querySelector(".tp__roll") ?? skinTp?.querySelector("circle");
  ...
  function updateTpRoll() {
    ...
    const r = Math.max(TP_CORE_R, Math.round(TP_FULL_R - ratio * (TP_FULL_R - TP_CORE_R)));
    tpRollCircle.setAttribute("r", r);
  }

  // 뽑힌 횟수에 따라 꼬랑지(늘어진 종이) 길이 갱신
  ...
  function handleLatchClick() {
    ...
    const now = Date.now();
    latchEasterTaps = latchEasterTaps.filter((t) => now - t < LATCH_EASTER_WINDOW_MS);
  ...
  function tapCeilingLight() {
    const now = Date.now();
    lightEasterTaps = lightEasterTaps.filter((t) => now - t < LIGHT_EASTER_WINDOW_MS);
    ...
    if (
      ...
      }, LIGHT_EASTER_BOT_DELAY_MS);
    }
  }

  // 오버레이가 있으면 오버레이로, 없으면 skinLight 직접으로 폴백
  if (ceilingLightTap) ceilingLightTap.addEventListener("click", tapCeilingLight);
  ...
  function tapGlobalChip() {
    ...
    const now = Date.now();
    devUnlockTaps = now - devUnlockTapAt <= DEV_TAP_RESET_MS ? devUnlockTaps + 1 : 1;
    devUnlockTapAt = now;
  ...
  return () => {
    ...
    if (ceilingLightTap) ceilingLightTap.removeEventListener("click", tapCeilingLight);
    else if (skinLight) skinLight.removeEventListener("click", tapCeilingLight);
    if (el.globalChip) el.globalChip.removeEventListener("click", tapGlobalChip);
