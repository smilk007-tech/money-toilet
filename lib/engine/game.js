...
export function initGame() {
  ...
  function restoreAwayProgress() {
    ...
    if (newSat >= MAX_SESSION_SECONDS) {
      setTimerCapped(true);
      toast("⏰ 자리 비운 사이 시급이 꽉 찼습니다!");
    } else if (awaySec >= 300) {
      // 5분 이상 자리비움 시에만 토스트
      const awayMin = Math.max(1, Math.round(awaySec / 60));
      toast(`🚪 부재중 자동 적립이 됐습니다! +${awayMin}분`);
    }
  }
  ...
  function showStallsLoading() {
    if (stallsOfflineTimer) { clearTimeout(stallsOfflineTimer); stallsOfflineTimer = null; }
  ...
  function persistGlobalForShare() {
    ...
    try {
      if (state.global > 0)
        sessionStorage.setItem("mt_game_global", String(Math.round(state.global)));
    } catch {}
  }
  ...
  try {
    ...
    if (_hs !== null) {
      ...
      if (isFinite(n) && n > 0) {
        el.stallCount.textContent = String(n);
        showStallsCount(); // 핸드오프 값이 있으면 로딩 상태 즉시 해제
      }
    }
  } catch {}
  // 설정 UI 초기화
