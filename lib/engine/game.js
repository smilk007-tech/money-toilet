...
export function initGame() {
  ...
  function restoreAwayProgress() {
    ...
    const credited = earnPerSecCeil() * creditedSec;
    if (newSat >= MAX_SESSION_SECONDS) {
      setTimerCapped(true);
      toast("⏰ 자리를 비운 사이 내 시급이 꽉 채워졌어요!");
    } else if (credited > 0) {
      const awayMin = Math.max(1, Math.round(awaySec / 60));
      toast(
        `🚪 자리를 비운 ${awayMin}분 동안 ${formatWon(credited)}을 벌었습니다!`,
      );
    }
  }
  ...
  function commitFlush(amount, chatText, capped) {
    ...
    socket.flush(
      amount,
      true,
      chatText || (capped ? pick(CONSTIPATION_LINES) : null),
  ...
  function getSideForUser(name) {
    ...
    const side = lc < rc ? "left" : rc < lc ? "right" : Math.random() < 0.5 ? "left" : "right";
  ...
  function showBubble(msg, forceSide) {
    ...
    const slot = msg.kind === "flush" || msg.kind === "capped" ? "flush" : "chat";
    ...
    const duration = msg.kind === "admin" ? ADMIN_BUBBLE_DURATION : BUBBLE_DURATION;
    setTimeout(() => {
      b.remove();
      if (userBubbles[msg.name]?.[slot] === b) delete userBubbles[msg.name][slot];
    }, duration);
  }
  ...
  // 소켓 연결 실패/복구 — 배지 표시/숨김 (RealToiletSocket에서만 발생)
  socket.on("offline", () => {
    if (el.networkBadge) el.networkBadge.hidden = false;
  });
  socket.on("online", () => {
    if (el.networkBadge) el.networkBadge.hidden = true;
  });
  ...
  // 탭 숨김/종료 직전 — 자리비움 보상 계산이 최대 1초 어긋나지 않게 즉시 영속
  document.addEventListener("visibilitychange", () => {
    if (document.visibilityState === "hidden") persistSatSeconds();
  });
