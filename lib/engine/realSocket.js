...
export class RealToiletSocket {
  ...
  /* ---- 공유하기 클릭 집계 ---- 클릭 즉시 fire-and-forget로 서버에 알림(집계 누수 방지).
     연결이 끊긴 상태여도 socket.io가 버퍼링 후 재연결 시 전송한다. created=자랑 URL 신규 생성. */
  shareClicked(created = false) {
    this._sock?.emit("share", { created: !!created });
  }
