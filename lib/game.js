...
export function initGame() {
  ...
  const FIRST_SEEN_KEY = "ddong_first_seen"; // 최초 브라우저 접속 시각(ms)
  const RECEIPT_REVEALED_KEY = "ddong_receipt_revealed"; // 영수증 버튼 등장 여부
  const RECEIPT_REVEAL_DELAY = 60_000; // 최초 접속 후 1분 지나야 영수증 버튼 등장
  const TOTAL_VISIBLE_KEY = "ddong_total_visible"; // "총 N원" 영역 표시 여부
  const TOTAL_REVEAL_DELAY = 5000; // 첫 물내림 후 5초 뒤에 표시
  const ADAPTATION_TIME = 3000; // 진입 후 최초 적응시간(ms)
  let firstSeenAt = 0; // 최초 접속 시각(init에서 로드/기록)
  let firstFlushTime = 0; // 첫 물내림 시각
  ...
  function loadAll() {
    ...
    state.receiptRevealed = LS.get(RECEIPT_REVEALED_KEY, "0") === "1";
    state.totalVisible = LS.get(TOTAL_VISIBLE_KEY, "0") === "1";
  }
  ...
  function revealTotal() {
    ...
    LS.set(TOTAL_VISIBLE_KEY, "1");
    renderTotal(); // --visible 부여(최종 상태 opacity 1)
    // 최초 1회성 팝업 애니메이션
    const node = el.totalEarned;
    node.classList.remove("deckcol__total--pop");
    void node.offsetWidth; // 리플로우 강제 → 애니메이션 재시작
    node.classList.add("deckcol__total--pop");
  ...
  // 최초 브라우저 접속 시각을 로컬스토리지에서 읽고, 없으면 지금으로 기록
  function getFirstSeen() {
    let t = parseInt(LS.get(FIRST_SEEN_KEY, ""), 10);
    if (!Number.isFinite(t) || t <= 0) {
      t = Date.now();
      LS.set(FIRST_SEEN_KEY, String(t));
    }
    return t;
  }
  // 영수증 버튼 노출. reveal=true 면 최초 등장 연출(팝인+글로우, 토스트는 띄우지 않음)
  function showReceiptBtn(reveal) {
    const btn = el.receiptBtn;
    if (!btn) return;
    btn.hidden = false;
    if (reveal) {
      btn.classList.remove("flush-receipt--reveal");
      void btn.offsetWidth; // 리플로우 강제 → 애니메이션 재시작
      btn.classList.add("flush-receipt--reveal");
  ...
  // 최초 접속 후 1분 이상 경과 && 물내림 2회차 이상일 때 영수증 버튼 등장
  function maybeRevealReceipt() {
    if (state.receiptRevealed) return; // 이미 등장함
    if (Date.now() - firstSeenAt < RECEIPT_REVEAL_DELAY) return; // 아직 1분 안 됨
    if (state.flushCount < 2) return; // 물내림 2회차 미만
    state.receiptRevealed = true;
    LS.set(RECEIPT_REVEALED_KEY, "1");
    showReceiptBtn(true);
  }
  ...
  function flush() {
    ...
    if (isFirstFlush) {
      setTimeout(revealTotal, TOTAL_REVEAL_DELAY);
    }
    // 물내림 5회마다 급여명세서 버튼 흔들기 (버튼이 보여지는 경우에만, 5초 후)
    if (state.flushCount % 5 === 0 && !el.receiptBtn.hidden) {
      setTimeout(nudgeReceiptBtn, TOTAL_REVEAL_DELAY);
    }
  }
  ...
  firstSeenAt = getFirstSeen();
  adaptationEndTime = Date.now() + ADAPTATION_TIME;
