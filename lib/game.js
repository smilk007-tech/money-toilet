...
export function initGame() {
  ...
  const state = {
    ...
    salaryHintDismissed: false, // 월급 버튼 변경 힌트(✏️) 숨김 여부
  };
  ...
  const KEY = {
    ...
    salaryHintDismissed: "ddong_salary_hint_dismissed",
  };
  ...
  const FLUSH_NUDGE_DELAY = 5000; // 5회마다 영수증 버튼 흔들기까지 지연
  ...
  let salaryPanelOpened = false; // 이번에 월급 팝오버를 연 적 있음(닫을 때 힌트 제거 판정)
  ...
  function commitFlush(amount, chatText) {
    ...
    // 물내림 5회마다 급여명세서 버튼 흔들기 (버튼이 보여지는 경우에만, 5초 후)
    if (state.flushCount % 5 === 0 && !el.receiptBtn.hidden)
  ...
  function renderSalaryHint() {
    ...
    el.salaryChangeHint.hidden = state.salaryHintDismissed;
  }
  function dismissSalaryHint() {
    if (state.salaryHintDismissed) return;
    state.salaryHintDismissed = true;
    LS.setBool(KEY.salaryHintDismissed, true);
    renderSalaryHint();
  }

  /* ---------- 팝오버 ---------- */
  function closeSalaryPanel() {
    if (!el.salaryPanel.hidden && salaryPanelOpened) dismissSalaryHint();
  ...
  function togglePanel(name) {
    ...
    if (panel.hidden) {
      closePanels(name);
      panel.hidden = false;
      if (name === "salary") salaryPanelOpened = true;
    } else {
      closePanels();
    }
  }
  ...
  el.salaryToggle.addEventListener("click", () => togglePanel("salary"));
