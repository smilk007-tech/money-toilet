...
export function initGame() {
  ...
  const RECEIPT_REVEALED_KEY = "ddong_receipt_revealed"; // 영수증 버튼 등장 여부(영속)
  const RECEIPT_REVEAL_DELAY = 60_000; // 이번 세션 진입 후 1분 지나야 노출대상
  ...
  let sessionStartAt = 0; // 이번 세션(스크립트) 시작 시각 — 1분 경과 판정용(LS 미사용)
  let receiptRevealScheduled = false; // 영수증 최초 등장 예약 여부(중복 예약 방지)
  ...
  // 노출대상이 되면 물내림 5초 후 애니메이션으로 최초 등장(이후 RECEIPT_REVEALED_KEY로 영속).
