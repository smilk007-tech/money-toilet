...
// 월급(원) → 초당 수입
const perSec = (salary) => salary / (CONFIG.workDaysPerMonth * CONFIG.workHoursPerDay * 3600);
...
const uniqueFrontWords = [
  "3사로", "2층끝칸", "5분지난", "4층둘째칸", "갈곳잃은", "날로먹는", "삘받은", "1층화장실",
  "심심한", "기름진", "기도하는", "기척없는", "긴급상황", "과민대장", "데이터다쓴", "방귀대장",
  "김밥먹는", "꽉막힌", "숙면중인", "내려놓은", "넷플보는", "노캔중인", "농땡이", "장염걸린",
  "눈치보는", "느낌좋은", "뉴스보는", "단골손님", "단타왕", "매우급한", "오래참은", "뛰어온", "댓글다는",
  "드러누운", "뚜껑내린", "과음한", "엄청쌓인", "많이모은", "표류중인", "매일출석", "해탈한",
  "멍때리는", "시끄러운", "못끊는", "피곤한", "몰래나온", "반응없는", "무아지경", "퇴사각",
  "물티슈왕", "밥값하는", "방금온", "변기위의", "부끄러운", "자랑하는", "귀밝은", "진심인", "목숨건",
  "부여잡은", "불경외는", "비데필수", "사람냄새", "십분초과", "고액연봉", "최저임금",
  "상쾌한", "색깔좋은", "소설읽는", "숏폼중독", "숙취심한", "숨바꼭질", "노래하는", "통화중인",
  "시말서쓴", "시원한", "쑥내려간", "안나가는", "에어팟낀", "집가고픈", "지박령",
  "영혼없는", "월급루팡", "현자가된", "딱걸린", "누리고픈", "흥분한",
  "웹툰주행", "유급휴가", "변기막힌", "사라진", "유튜브시청", "융단폭격", "응원단장", "이어폰낀", "인생무상", "인스타중",
  "일퀘하는", "자리비운", "자체복지", "잠수중인", "중독된", "적립왕", "전설이된", "전세낸", 
  "넘쳐버린", "조퇴각", "식곤증온", "원격근무", "존버타는", "주식보는", "배아픈",
  "외근나간", "출장중인", "업비트중", "차트보는", "천둥치는", "천만원번", "밤샘근무", "추노한",
  "칼퇴고픈", "커뮤중인", "뚜껑덮은", "커피값번", "조난당한", "양심적인", "물내리는", "파업예고",
  "과자먹는", "쾌변달인", "쿠키충전", "큰일난", "절박한", "투명인간",
  "루틴강박", "평생회원", "엉뜨필수", "풀배터리", "변비걸린", "향기로운", "무색무취",
  "황금빛깔", "황홀한", "뛰쳐나온", "즉흥적인", "휴지없는", "그냥와본", "싹다비운", "잘만드는", "한발늦은",
  "사장라인", "특급닌자", "다크템플러", "불법체류", "모범직원", "백두혈통", "뻔뻔한",
  "진급대상", "못참는", "도망자",
];
...
const SURNAMES = [
  ...["김","이","박","최","정","강","조","윤","장","임","한","오","서","신","권","황","안","송","전","홍","유","고"].map((name) => ({ name, tier: "상" })),
  ...["문","양","손","배","백","허","남","심","노","하","곽","성","차","주","우","구","나","민","진","지","엄","채","원","천","방","공","현","함","변","염","여","추","동","석","선","설","마","길"].map((name) => ({ name, tier: "중" })),
  ...["표","명","기","반","왕","금","옥","육","맹","모","탁","은","편","봉","예"].map((name) => ({ name, tier: "하" })),
];
...
function weightedPick(items) {   // items: [{value, weight}]
  const total = items.reduce((s, it) => s + it.weight, 0);
  let r = Math.random() * total;
  for (const it of items) { if (r < it.weight) return it.value; r -= it.weight; }
  return items[items.length - 1].value;
}
const pickSurname = () => weightedPick(SURNAMES.map((s) => ({ value: s.name, weight: SURNAME_TIER_WEIGHT[s.tier] })));
const pickRank = () => pick(weightedPick(RANK_GROUPS.map((g) => ({ value: g.ranks, weight: g.weight }))));
const randomNickname = () => `${pick(FRONT_WORDS)} ${pickSurname()}${pickRank()}`.slice(0, 10);

/* ---------- 포맷 ---------- */
const wonNum = (n) => Math.ceil(n).toLocaleString("ko-KR") + "원";          // n원 (₩ 없음)
const salaryText = (v) => (v === 0 ? "휴식중" : v >= 100_000_000 ? "1억원" : (v / 10000) + "만원");  // 쉼표 없이 짧게
const mmss = (s) => `${String(Math.floor(s / 60)).padStart(2, "0")}:${String(s % 60).padStart(2, "0")}`;
function pick(a) { return a[Math.floor(Math.random() * a.length)]; }
function uniqSortKo(arr) { return [...new Set(arr)].sort((a, b) => a.localeCompare(b, "ko")); }
const perSecInt = () => Math.ceil(state.earnRate);   // "1초에 N원" (정수, 올림)

/* ---------- DOM ---------- */
const $ = (id) => document.getElementById(id);
const el = {
  stallCount: $("stallCount"), globalEarned: $("globalEarned"), globalChip: document.querySelector(".hud__global"),
  personalEarned: $("personalEarned"), rateLabel: $("rateLabel"), timer: $("timer"), timerVal: $("timerVal"), totalEarned: $("totalEarned"),
  flushBtn: $("flushBtn"),
  salaryToggle: $("salaryToggle"), salaryLabel: $("salaryLabel"), salaryPanel: $("salaryPanel"),
  salaryRange: $("salaryRange"), salaryBig: $("salaryBig"), salaryRate: $("salaryRate"),
  gearBtn: $("gearBtn"), settingsPanel: $("settingsPanel"), nickInput: $("nickInput"),
  nickRandomBtn: $("nickRandomBtn"), nickPinChk: $("nickPinChk"),
  totalToggle: $("totalToggle"), timerToggle: $("timerToggle"), resetTotalBtn: $("resetTotalBtn"),
  chatForm: $("chatForm"), chatInput: $("chatInput"),
  bubblesLeft: $("bubblesLeft"), bubblesRight: $("bubblesRight"), myFeed: $("myFeed"),
  confettiLayer: $("confettiLayer"), toast: $("toast"),
  adA4: $("adA4"), adEmoji: $("adEmoji"), adHead: $("adHead"), adSub: $("adSub"),
...
function loadAll() {
  ...
  state.nick = (state.nickPinned && savedNick) ? savedNick.slice(0, 10) : randomNickname();
  // 합계 / 토글
  state.totalEarned = Math.max(0, Number(LS.get("ddong_total", 0)) || 0);
  state.flushCount = Math.max(0, parseInt(LS.get("ddong_flushcount", "0"), 10) || 0);
...
function nearestStepIndex(v) {
  ...
  SALARY_STEPS.forEach((s, i) => { const d = Math.abs(s - v); if (d < bd) { bd = d; bi = i; } });
  return bi;
}
...
/* ---------- 표시 ---------- */
const tabularWon = (s) => s.replace(/[0-9]/g, (d) => `<span class="digit">${d}</span>`);
function renderPersonal(tick) {
  el.personalEarned.innerHTML = tabularWon(wonNum(state.personal));   // ₩ 대신 N원, 숫자만 고정폭
  if (tick) { el.personalEarned.classList.remove("tick"); void el.personalEarned.offsetWidth; el.personalEarned.classList.add("tick"); }
}
function renderGlobal(flash) {
  el.globalEarned.textContent = wonNum(state.global);
  if (flash) { el.globalChip.classList.remove("flash"); void el.globalChip.offsetWidth; el.globalChip.classList.add("flash"); }
}
function renderTimer() { el.timerVal.textContent = mmss(state.satSeconds); }
function renderTotal() { el.totalEarned.textContent = `총 ${wonNum(state.totalEarned)}`; }
...
/* ---------- 물내리기 = 정산 ---------- */
function meName() { return `${state.nick} (나)`; }

function resetTimer() {
  state.satSeconds = 0; state.personal = 0; renderPersonal(); if (state.timer) renderTimer();
...
function flush() {
  ...
  if (state.salary === 0) { toast(pick(REST_LINES)); return; }
  const amount = Math.ceil(state.personal);
  if (amount < 1) return;
  state.totalEarned += amount; LS.set("ddong_total", state.totalEarned); renderTotal();
  state.flushCount += 1; LS.set("ddong_flushcount", state.flushCount);
  resetTimer();
  confettiBurst(); flushSound();
...
function autoFlush() {
  const amount = Math.ceil(state.personal);
  resetTimer();
  if (state.salary === 0 || amount < 1) { toast(pick(REST_LINES)); return; }   // 무수입이면 그냥 리셋만
  state.totalEarned += amount; LS.set("ddong_total", state.totalEarned); renderTotal();
  state.flushCount += 1; LS.set("ddong_flushcount", state.flushCount);
  confettiBurst(); flushSound();
...
function confettiBurst() {
  ...
  for (let i = 0; i < 18; i++) {
    ...
    c.className = "confetti"; c.textContent = pick(emojis);
  ...
  setTimeout(() => { el.confettiLayer.innerHTML = ""; }, CONFETTI_DURATION + 1200);
}
...
function placeBubble(layer, b) {
  const H = layer.clientHeight, h = b.offsetHeight, maxTop = Math.max(0, H - h);
  const occ = [...layer.children].filter((x) => x !== b && x.style.top)
    .map((x) => { const t = parseFloat(x.style.top) || 0; return [t, t + x.offsetHeight]; });
  let best = 0, bestOv = Infinity;
  for (let t = 0; t <= maxTop; t += 6) {
    let ov = 0;
    for (const [a, c] of occ) { const o = Math.min(t + h, c) - Math.max(t, a); if (o > 0) ov += o; }
    if (ov === 0) { best = t; bestOv = 0; break; }
...
function showMyMessage(text, kind = "chat") {
  ...
  setTimeout(() => { m.classList.add("myfeed__msg--out"); setTimeout(() => m.remove(), 500); }, MY_MSG_DURATION);
}
...
function toast(text) {
  ...
  clearTimeout(toastTimer); toastTimer = setTimeout(() => (el.toast.hidden = true), TOAST_DURATION);
}
...
socket.on("presence", ({ count }) => { state.stalls = count; el.stallCount.textContent = count; });
socket.on("global", ({ total }) => { state.global = total; renderGlobal(); });
socket.on("chat", (msg) => {
  if (msg.kind === "system") return;        // 입장 알림: 기능만 유지, 표시 보류
  if (msg.kind === "me") { showMyMessage(msg.text, "chat"); return; }   // 내 채팅 → 덱 위 피드(민트)
  showBubble({ name: msg.name, text: msg.text, kind: msg.kind });
});
socket.on("flush", (f) => {
  state.global = f.total; renderGlobal(true);
  if (f.me) { if (f.chat) showMyMessage(f.text || flushBrag(f.amount), "flush"); return; }  // 내 정산 → 피드(노랑)
  showBubble({ name: f.name, text: f.text || flushBrag(f.amount), kind: "flush" });
});
...
function finalizeNickname() {
  ...
  if (state.nickPinned) LS.set("ddong_nick", v); else LS.remove("ddong_nick");
}

/* ---------- 팝오버 ---------- */
function closeSalaryPanel() { el.salaryPanel.hidden = true; }
function closeSettingsPanel() {
  if (el.settingsPanel.hidden) return;
  finalizeNickname();
  el.settingsPanel.hidden = true;
  document.getElementById("nickRandomIcon").classList.remove("nick-random--spin");
}
...
el.salaryToggle.addEventListener("click", () => {
  ...
  if (willOpen) { closePanels("salary"); el.salaryPanel.hidden = false; }
  else { closeSalaryPanel(); }
});
el.salaryRange.addEventListener("input", (e) => applySalary(SALARY_STEPS[Number(e.target.value)]));
el.salaryRange.addEventListener("change", (e) => { applySalary(SALARY_STEPS[Number(e.target.value)]); closeSalaryPanel(); });

el.gearBtn.addEventListener("click", () => {
  const willOpen = el.settingsPanel.hidden;
  if (willOpen) { closePanels("settings"); el.settingsPanel.hidden = false; }
  else { closeSettingsPanel(); }
});
// 팝오버 외 영역 클릭 시 닫기
document.addEventListener("click", (e) => {
  if (!el.salaryPanel.hidden && !el.salaryPanel.contains(e.target) && !el.salaryToggle.contains(e.target)) closeSalaryPanel();
  if (!el.settingsPanel.hidden && !el.settingsPanel.contains(e.target) && !el.gearBtn.contains(e.target)) closeSettingsPanel();
});

el.nickInput.addEventListener("input", (e) => { state.nick = e.target.value.slice(0, 10); });
el.nickRandomBtn.addEventListener("click", () => {
  ...
  icon.addEventListener("animationend", () => icon.classList.remove("nick-random--spin"), { once: true });
});
el.nickPinChk.addEventListener("change", (e) => {
  state.nickPinned = e.target.checked;
  LS.set("ddong_nickpin", state.nickPinned ? "1" : "0");
  if (state.nickPinned) LS.set("ddong_nick", state.nick); else LS.remove("ddong_nick");
});
el.totalToggle.addEventListener("change", (e) => {
  state.showTotal = e.target.checked; LS.set("ddong_showtotal", state.showTotal ? "1" : "0");
  el.totalEarned.hidden = !state.showTotal; el.resetTotalBtn.hidden = !state.showTotal; renderTotal();
});
el.timerToggle.addEventListener("change", (e) => {
  state.timer = e.target.checked; LS.set("ddong_timer", state.timer ? "1" : "0");
  el.timer.hidden = !state.timer; if (state.timer) renderTimer();
});
el.resetTotalBtn.addEventListener("click", () => { state.totalEarned = 0; LS.set("ddong_total", 0); renderTotal(); toast("내가 번 돈 합계 초기화 완료 🧹"); });
...
el.settingsLinks.forEach((btn) => {
  btn.addEventListener("click", () => toast(SETTINGS_LINK_MESSAGES[btn.dataset.action] || "준비중입니다"));
});
...
el.chatForm.addEventListener("submit", (e) => {
  ...
  if (!text) return;
  socket.send(text); el.chatInput.value = "";
});
el.adA4.addEventListener("click", (e) => { e.preventDefault(); toast("광고 문의: 이 자리에 실광고 연결 📢"); });
...
function buildReceiptCanvas() {
  const W = 400, PAD = 30;
  const INK = "#20271f", SUB = "#717a6f", LINE = "#c9ccc0", PAPER = "#fbfaf3", GOLD = "#a9760a";
  const font = (s, w = "400") => `${w} ${s}px "Courier New","Apple SD Gothic Neo","Pretendard",monospace`;
  ...
  const text = (ctx, s, x, y, ft, col, align) => {
    if (!ctx) return; ctx.font = ft; ctx.fillStyle = col; ctx.textAlign = align || "left";
    ctx.textBaseline = "alphabetic"; ctx.fillText(s, x, y);
  };
  const dash = (ctx, y) => {
    if (!ctx) return; ctx.strokeStyle = LINE; ctx.lineWidth = 1.2; ctx.setLineDash([5, 4]);
    ctx.beginPath(); ctx.moveTo(PAD, y); ctx.lineTo(W - PAD, y); ctx.stroke(); ctx.setLineDash([]);
  };
  const solid = (ctx, y, w, col) => {
    if (!ctx) return; ctx.strokeStyle = col || INK; ctx.lineWidth = w || 1.5;
    ctx.beginPath(); ctx.moveTo(PAD, y); ctx.lineTo(W - PAD, y); ctx.stroke();
  };
  const barcode = (ctx, yTop, h) => {
    if (!ctx) return;
    ctx.fillStyle = INK;
    let x = PAD, seed = (hero * 2654435761 + 12345) >>> 0;
  ...
  function paint(ctx) {
    let y = 14;                                                            // 상단 톱니 여백
    y += 32; text(ctx, "💩 똥탐", W / 2, y, font(32, "800"), INK, "center");
    y += 20; text(ctx, "화 장 실  영 수 증", W / 2, y, font(13, "700"), SUB, "center");
    y += 12; dash(ctx, y);
    y += 22; text(ctx, "발행", PAD, y, font(12), SUB); text(ctx, issued, W - PAD, y, font(12), INK, "right");
    y += 20; text(ctx, "손님", PAD, y, font(12), SUB); text(ctx, nick, W - PAD, y, font(12, "700"), INK, "right");
    y += 12; dash(ctx, y);
    y += 22; text(ctx, "품목", PAD, y, font(11), SUB); text(ctx, "금액", W - PAD, y, font(11), SUB, "right");
    y += 25; text(ctx, "화장실 근무수당", PAD, y, font(13), INK); text(ctx, f(settled), W - PAD, y, font(13), INK, "right");
    y += 25; text(ctx, "실시간 적립(진행중)", PAD, y, font(13), INK); text(ctx, f(live), W - PAD, y, font(13), INK, "right");
    y += 12; solid(ctx, y, 1.6, INK);
    y += 24; text(ctx, "합계", PAD, y, font(15, "800"), INK); text(ctx, "TOTAL", PAD + 48, y, font(10), SUB);
    y += 42; text(ctx, f(hero), W / 2, y, font(38, "800"), INK, "center");
    y += 18; solid(ctx, y, 1.2, INK); solid(ctx, y + 3, 1.2, INK);
    y += 28; text(ctx, "오늘 다 같이 번 돈", W / 2, y, font(12, "700"), SUB, "center");
    y += 26; text(ctx, f(global), W / 2, y, font(20, "800"), GOLD, "center");
    y += 24; text(ctx, `지금 볼일 중  ${stalls}명   ·   내 물내림  ${flushes}회`, W / 2, y, font(11, "600"), SUB, "center");
    y += 14; dash(ctx, y);
    y += 28; text(ctx, `"${slogan}"`, W / 2, y, font(13, "700"), INK, "center");
    y += 22; barcode(ctx, y, 38);
    y += 38 + 20; text(ctx, "똥탐 · paid-toilet", W / 2, y, font(12, "700"), INK, "center");
    y += 18; text(ctx, SHARE_URL.replace(/^https?:\/\//, ""), W / 2, y, font(10), SUB, "center");
    y += 16;
    return y;
  }
  ...
  ctx.beginPath();
  ctx.moveTo(0, d);
  for (let x = 0; x < W; x += tooth) { ctx.lineTo(x + tooth / 2, 0); ctx.lineTo(x + tooth, d); }
  ctx.lineTo(W, H - d);
  for (let x = W; x > 0; x -= tooth) { ctx.lineTo(x - tooth / 2, H); ctx.lineTo(x - tooth, H - d); }
  ctx.closePath();
...
function openReceipt() {
  ...
  prev.innerHTML = ""; prev.appendChild(currentReceiptCanvas);
...
function receiptBragText() {
  const hero = Math.max(0, state.totalEarned) + Math.max(0, Math.ceil(state.personal));
  return `💩 화장실에서 ${f0(hero)} 벌었다 ㅋㅋ #똥탐`;
}
// 영수증 스냅샷을 URL에 담은 공유 링크 (SNS에서 OG 카드로 펼쳐짐). 월급/시간은 안 담음.
function buildShareUrl() {
  const settled = Math.max(0, state.totalEarned);
  const live = Math.max(0, Math.ceil(state.personal));
  const stalls = state.stalls || parseInt(el.stallCount.textContent, 10) || 0;
  const d = encodeReceipt({
    n: (state.nick || "익명의 볼일러").slice(0, 16),
    s: settled, l: live, g: Math.max(0, state.global),
    p: stalls, f: state.flushCount || 0, ts: Date.now(), sl: currentReceiptSlogan,
  });
  return `${location.origin}/r/${d}`;
}

function canvasToBlob(c) {
  return new Promise((res) => { try { c.toBlob((b) => res(b), "image/png"); } catch (e) { res(null); } });
}
...
async function shareReceipt() {
  ...
  if (navigator.share) {
    try { await navigator.share({ title: "똥탐 영수증", text, url }); return; }
...
async function saveReceipt() {
  ...
  if (blob) { downloadBlob(blob, "ddongtam-receipt.png"); toast("영수증 이미지를 저장했어요 🧾"); }
}
...
el.totalEarned.hidden = !state.showTotal; el.resetTotalBtn.hidden = !state.showTotal; renderTotal();
el.timer.hidden = !state.timer; renderTimer();
rotateAd(); const adInterval = setInterval(rotateAd, 6000);
socket.connect();
