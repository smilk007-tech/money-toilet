...
export function initGame() {
  ...
  // 모바일 키보드 — UI는 레이아웃 위치 고정, 말풍선만 가시 영역에 맞춤
  ...
  function relayoutBubbles() {
    for (const layer of [el.bubblesLeft, el.bubblesRight]) {
      if (!layer) continue;
      for (const b of layer.children) {
        if (b.style.top) placeBubble(layer, b);
      }
    }
  }
  ...
  function syncMobileViewport() {
    ...
    document.documentElement.style.setProperty("--vv-offset", `${offsetY}px`);

    if (!el.bubblesLeft || !el.bubblesRight) return;
    const appRect = appEl.getBoundingClientRect();
    const hudBottom = hudEl?.getBoundingClientRect().bottom ?? appRect.top;
    const visibleBottom = vv
      ? Math.min(vv.offsetTop + vv.height, window.innerHeight)
      : appRect.bottom;
    const top = Math.max(8, hudBottom - appRect.top + 4);
    const bottomGap = Math.max(8, appRect.bottom - visibleBottom + 4);
    for (const layer of [el.bubblesLeft, el.bubblesRight]) {
      layer.style.top = `${top}px`;
      layer.style.bottom = `${bottomGap}px`;
    }
    relayoutBubbles();
  }

  function scheduleMobileViewportSync() {
    cancelAnimationFrame(bubbleSyncRaf);
    bubbleSyncRaf = requestAnimationFrame(syncMobileViewport);
  }

  const onMobileViewport = () => scheduleMobileViewportSync();
  const onChatFocus = () => {
    scheduleMobileViewportSync();
    setTimeout(scheduleMobileViewportSync, 120);
    setTimeout(scheduleMobileViewportSync, 360);
  };
  const onChatBlur = () => setTimeout(scheduleMobileViewportSync, 120);
  const onOrientation = () => {
    setTimeout(() => {
      lockAppHeight();
      scheduleMobileViewportSync();
  ...
  if (window.visualViewport) {
    window.visualViewport.addEventListener("resize", onMobileViewport);
    window.visualViewport.addEventListener("scroll", onMobileViewport);
  }
  window.addEventListener("orientationchange", onOrientation);
  window.addEventListener("resize", onMobileViewport);
  window.addEventListener("scroll", onWindowScroll, { passive: true });
  el.chatInput?.addEventListener("focus", onChatFocus);
  el.chatInput?.addEventListener("blur", onChatBlur);
  scheduleMobileViewportSync();
  ...
  return () => {
    ...
    if (window.visualViewport) {
      window.visualViewport.removeEventListener("resize", onMobileViewport);
      window.visualViewport.removeEventListener("scroll", onMobileViewport);
    }
    window.removeEventListener("orientationchange", onOrientation);
    window.removeEventListener("resize", onMobileViewport);
    window.removeEventListener("scroll", onWindowScroll);
    el.chatInput?.removeEventListener("focus", onChatFocus);
    el.chatInput?.removeEventListener("blur", onChatBlur);
    cancelAnimationFrame(bubbleSyncRaf);
