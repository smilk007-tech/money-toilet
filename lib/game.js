...
export function initGame() {
  ...
  const KEY = {
    ...
    timer: "ddong_timer",
    timerAutoShown: "ddong_timer_auto_shown", // 자동 ON 발동 여부(최초 1회만)
    timerTouched: "ddong_timer_touched", // 설정에서 타이머 표시를 수동으로 건드린 적 있는지
    salaryFocusDone: "ddong_salary_focus_done", // 내 월급 팝퍼 최초 1회 집중 모드 소진 여부
    settingsFocusDone: "ddong_settings_focus_done", // 설정 팝퍼 최초 1회 집중 모드 소진 여부
    payslipConfirmed: "ddong_payslip_confirmed", // 급여명세서 지급완료 확인 넛지 소진 여부(PayslipModal.tsx와 키 공유)
    everVisited: "ddong_ever_visited", // 이 브라우저로 한 번이라도 들어온 적 있는지(진입 그레이스 타임 산정용)
  ...
  // 개발용 — 이 기기의 ddong_* 로컬스토리지를 전부 비우고 새로고침(넛지/진행도 전부 리셋)
  if (el.devWipeAllBtn) {
  ...
  window.addEventListener("ddong:payslip-stamped", revealReceiptFromStamp);
  ...
  return () => {
    ...
    window.removeEventListener("ddong:payslip-stamped", revealReceiptFromStamp);
    try {
      socket.disconnect();
    } catch (e) {}
  };
