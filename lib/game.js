...
export function initGame() {
  ...
  const KEY = {
    salary: "ddong_salary",
    nick: "ddong_nick",
    nickPin: "ddong_nickpin",
    total: "ddong_total",
    flushCount: "ddong_flushcount",
    flushHistory: "ddong_flush_history",
    timer: "ddong_timer",
    receiptRevealed: "ddong_receipt_revealed",
    totalVisible: "ddong_total_visible",
    salaryHintDismissed: "ddong_salary_hint_dismissed", // 구버전 — 마이그레이션용
    salaryClicked: "ddong_salary_clicked",
  };
  const FLUSH_HISTORY_MAX = 10; // 명세서 표시/저장 상한
  const SALARY_HINT_DELAY = 30_000; // 최초 진입 30초 후, 미클릭 시 👈 노출
  const RECEIPT_REVEAL_DELAY = 60_000; // 세션 진입 후 1분 지나야 영수증 노출대상
  ...
  const CHAT_SPAM_WINDOW_MS = 3000;
  const CHAT_SPAM_MUTE_MS = 10_000;
  const CHAT_SIMILARITY_WINDOW_MS = 10_000;
  const CHAT_SIMILARITY_THRESHOLD = 0.84;
  const CHAT_RECENT_MAX = 8;
  ...
  function loadAll() {
    ...
    state.timer = LS.getBool(KEY.timer, true);
  ...
  function renderTotal() {
    ...
    if (el.settingsHistoryGroup) {
      el.settingsHistoryGroup.hidden = !state.totalVisible;
    }
  }
  ...
  // 이번 세션 진입 후 1분 경과 && 로컬 물내림 횟수 2회 이상이면 노출대상.
  // (1회로 하면 신규유저가 1분 뒤 첫 물내림 때 "총 N원"과 영수증이 동시에 튀어나옴 → 2회로 분리)
  ...
  function maybeRevealReceipt() {
    ...
    if (Date.now() - sessionStartAt < RECEIPT_REVEAL_DELAY) return; // 세션 진입 1분 미만
    if (state.flushCount < 2) return; // 로컬 물내림 2회 미만
  ...
  function commitFlush(amount, chatText) {
    ...
    if (!el.receiptBtn.hidden)
      setTimeout(nudgeReceiptBtn, FLUSH_NUDGE_DELAY);
  }

  // 수동 물내림(버튼)
  function flush() {
    if (el.flushBtn.disabled) return;
    if (state.salary === 0) return void toast(pick(REST_LINES));
  ...
  function similarity(a, b) {
    const aa = normalizeChatText(a);
    const bb = normalizeChatText(b);
    if (!aa || !bb) return 0;
    const aaKind = classifyShortPattern(aa);
    const bbKind = classifyShortPattern(bb);
    if (aaKind && aaKind === bbKind) return 1;
  ...
  function rememberChatMessage(text, at) {
    ...
    chatGuard.recentMessages = chatGuard.recentMessages
      .filter((item) => at - item.at <= CHAT_SIMILARITY_WINDOW_MS)
  ...
  function shouldBlockChat(text) {
    ...
    const recent = chatGuard.recentMessages.filter(
      (item) => now - item.at <= CHAT_SIMILARITY_WINDOW_MS,
    );
    const recent3s = recent.filter((item) => now - item.at <= CHAT_SPAM_WINDOW_MS);

    if (recent3s.length >= 4) {
      return {
        blocked: true,
        reason: "3초 안에 채팅을 5번 이상 보내서 10초간 채팅 금지예요",
        mute: true,
      };
    }
    ...
    if (sameCount >= 3) {
      return {
        blocked: true,
        reason: "3초 안에 같은 말을 3번 반복해서 10초간 채팅 금지예요",
        mute: true,
      };
    }
    ...
    if (sameKindCount >= 3) {
      return {
        blocked: true,
        reason: "비슷한 웃음/감탄 채팅을 반복해서 10초간 채팅 금지예요",
        mute: true,
      };
    }
    ...
    const similarCount =
      recent.filter(
        (item) => item.norm && similarity(item.norm, norm) >= CHAT_SIMILARITY_THRESHOLD,
      ).length + 1;
    if (similarCount >= 3) {
      return {
        blocked: true,
        reason: "비슷한 말을 반복해서 10초간 채팅 금지예요",
        mute: true,
      };
    }
  ...
  function rotateAd() {
    ...
    el.adTag.textContent = currentAdCreative.tag;
  ...
  function hideSalaryHint(animate = false) {
    ...
    badge.classList.remove("ctrl-salary__badge--show", "ctrl-salary__badge--bounce");
    badge.style.opacity = "";
  ...
  function showSalaryHint() {
    ...
    const badge = el.salaryChangeHint;
    badge.hidden = false;
    badge.classList.remove("ctrl-salary__badge--hide", "ctrl-salary__badge--bounce");
  ...
  el.chatForm.addEventListener("submit", (e) => {
    ...
    if (guard.blocked) {
      if (guard.mute) muteChat(guard.reason);
      else toast(guard.reason);
      return;
    }
    socket.send(text);
