...
async function shareReceipt() {
  ...
  const file = blob ? new File([blob], "ddongtam-receipt.png", { type: "image/png" }) : null;
  ...
  // 2) 텍스트/링크만이라도 공유
  if (navigator.share) {
    try { await navigator.share({ text: txt, url: SHARE_URL }); return; }
    catch (e) { if (e && e.name === "AbortError") return; }
  }
...
rotateAd(); setInterval(rotateAd, 6000);
socket.connect();
